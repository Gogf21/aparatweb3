...
def validate_form_data(data: dict) -> dict:
    ...
    if error := validate_fullname(data.get('fullname', '')):
        errors['fullname'] = error
    
    if error := validate_phone(data.get('phone', '')):
        errors['phone'] = error
    
    if error := validate_email(data.get('email', '')):
        errors['email'] = error
    
    if error := validate_birthdate(data.get('birthdate', '')):
        errors['birthdate'] = error
    
    if error := validate_gender(data.get('gender', '')):
        errors['gender'] = error
    
    if error := validate_languages(data.get('language', [])):
        errors['language'] = error
    
    if error := validate_biography(data.get('bio', '')):
        errors['bio'] = error
    
    if error := validate_contract(data.get('contract', '')):
